(function() {
  ...
  /** @ngInject */
  function line(lineChart, lineChartIndexModel, econometricModels, expert, citizen, prediction, pollagg, indexModelsubcategory, pollssubcomponent, expertsubcategory, subcomponentCitizen, pmsubcomponent, ecosubcomponent, csvService) {
    return {
      restrict: 'E',
    ...
    function link($scope, element, $document) {
      ...
      function parseValues(data) {
        if (typeof(data) != 'number') {
          ...
          return parseFloat((+(Math.round(+(value + 'e' + _nPrecision)) + 'e' + -_nPrecision)).toFixed(_nPrecision))
        } else {
          var value = data
          return parseFloat((+(Math.round(+(value + 'e' + _nPrecision)) + 'e' + -_nPrecision)).toFixed(_nPrecision))
        }
      ...
      lineChart.getData()
        .success(function(pollyvotedata) {
          ...
          $(function() {
            ...
            // pushSeries('Uncertainty Clinton confidence 75%', false, uncertinatiyClinton, 'cc')
            ...
            $scope.onChangeIntervals = function() {
              ...
              for (i = 0; i < $scope.confidenceIntervals.length; i++) {
                if ($scope.confidenceIntervals[i].model == true) {
                  showValues($scope.confidenceIntervals[i].name)

                } else if ($scope.confidenceIntervals[i].model == false) {
                  hideValues($scope.confidenceIntervals[i].name)
